...
static void IOOUTCALL prt_o142(UINT port, REG8 dat) {
	...
	if (!(prtIEEE_control & PARALLEL_IEEE_CONTROL_STROBE) && (dat & PARALLEL_IEEE_CONTROL_STROBE)/* && (prtIEEE_control & PARALLEL_IEEE_CONTROL_SELECTIN)*/) {
		COMMNG	prt;
	...
	if ((prtIEEE_control & PARALLEL_IEEE_CONTROL_INIT) && !(dat & PARALLEL_IEEE_CONTROL_INIT)) {
		COMMNG	prt;
		prt = cm_prt;
		if (prt != NULL) {
...
static REG8 IOINPCALL prt_i141(UINT port) {
	...
	COMMNG	prt;
	REG8 ret = prtIEEE_status | PARALLEL_IEEE_STATUS_NOBUSY | PARALLEL_IEEE_STATUS_NOERROR;
...
void printif_reset(const NP2CFG *pConfig) {
	...
	prtIEEE_mode = 0x00; // 謎: Undocでbit 4は1=拡張パラレルポートモード, 0=簡易セントロニクスモード（デフォルト）ということになっているが、Win2000は何も設定せずに拡張のポートへアクセスしに来る？？
	i14bData = 0xff;
	i14dData = 0x00;
	i14eData = 0x01;
